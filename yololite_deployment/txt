...
void draw_box(cv::Mat &cv_mat, std::vector<BoxInfo> &boxes, MatInfo &mmat_objection,cv::VideoWriter *vw)
{
/*
...
*/
    ...
    for (auto box : boxes)
    {
        ...
        cv::putText(cv_mat, text, pos, cv::FONT_HERSHEY_SIMPLEX, ((float)objh / (float)mmat_objection.maxSide)*5,
                    cv::Scalar(0, 0, 255), 1);
    }
#if use_camera
